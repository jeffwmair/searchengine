<?php
	...
	*/
	...
	function checkPass() {
		$dumbPass = "searchenginepass";
		$pass = $_POST['pass'];
		if ($pass == NULL) $pass = $_GET['pass'];
		return ($pass == $dumbPass);
	}
	...
	function getSkipTerms() {
		$terms = array();
		// array_push($terms, 'home');
		// array_push($terms, 'site');
		// array_push($terms, 'make');
		// array_push($terms, 'help');
		// array_push($terms, 'contact');
		// array_push($terms, 'search');
		// array_push($terms, 'web');
		// array_push($terms, 'view');
		// array_push($terms, 'work');
		return $terms;
	}
	...
	function getRequestData($arg) {
		$d = $_POST[$arg];
		if ($d == NULL) $d = $_GET[$arg];
		return $d;
	}
	
	function sqlQuerySingleVal($conn, $sql, $columnName, $default) {
		$res = mysqli_query($conn, $sql);
		if (!$res) {
			echo "Error in query $sql :" . mysqli_error($conn) . "\n";
		}
	...
	function convertSqlRowsToJson($rows) {
		...
		{
			if ($json != '[') $json .= ',';
			$json .= stripslashes(json_encode($row));
		}
		$json .= ']';
		return $json;
	}
	...
	function getStems() {
		$stems = array();
		array_push($stems, "account");
		array_push($stems, "aqua");
		array_push($stems, "biblio");
		array_push($stems, "brows");
		array_push($stems, "chronol");
		array_push($stems, "cognit");
		array_push($stems, "communic");
		array_push($stems, "comput");
		array_push($stems, "home");
		array_push($stems, "informat");
		array_push($stems, "program");
		array_push($stems, "sale");
		array_push($stems, "welcome");
		return $stems;
	}
